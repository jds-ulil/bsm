...
$('.search-form form').submit(function(){	
    ...
    $('#print_kriteria_transaksi').val($('#dailyTeller_kriteria_transaksi').val());
	$('#print_from_date').val($('#dailyTeller_from_date').val());	
	$('#print_to_date').val($('#dailyTeller_to_date').val());	
	$('#print_nama_pegawai').val($('#dailyTeller_nama_pegawai').val());	

    $('#telreport-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
    ...
    $this->widget('bootstrap.widgets.TbGridView', array(
       ...
       'columns'=>array(
           ...
           array (
                ...
                'value'=>'$data->status == 1? '
                         . 'CHtml::ajaxLink("ACC", '
                                        . 'Yii::app()->createUrl("/daily/accTel", array("id" =>$data[\'daily_teller_id\'])),'
                                        . 'array ('
                                        .   '"type" => "GET",'
                                        .   '"dataType" => "json",'
                                        .   '"complete" => "'.'function(data){if(data.responseText==\'sukses update\') $.fn.yiiGridView.update(\'telreport-grid\');}'.'",'
                                            . ')'
                                            . ')'
                . ': "-"',
            ), 
            array (
                ...
                'template' => '{delete}',
                'deleteButtonUrl'=>'Yii::app()->createUrl("/daily/deleteTel", array("id" =>$data[\'daily_teller_id\']))',
            )
           )	
       ));   
} else {
...
<?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm', array(
    'id'=>'csreport-print',
	'action'=>Yii::app()->createUrl('daily/printtel'),
	'method'=>'post',        
        'type'=>'horizontal',
        'htmlOptions'=> array(
            'target'=>"_blank",
...
<input name="dailyTeller[kriteria_transaksi]" id="print_kriteria_transaksi" type="hidden" />
<input name="dailyTeller[from_date]" id="print_from_date" type="hidden" />
<input name="dailyTeller[to_date]" id="print_to_date" type="hidden" />
<input name="dailyTeller[nama_pegawai]" id="print_nama_pegawai" type="hidden" />
<div class="form-actions">        	
        <?php $this->widget('bootstrap.widgets.TbButton', array(
                'buttonType'=>'submit',                
                'type'=>'danger',
