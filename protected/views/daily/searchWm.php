...
$('.search-form form').submit(function(){	
    ...
    $('#print_kriteria_nasabah').val($('#dailyWm_kriteria_nasabah').val());
	$('#print_from_date').val($('#dailyWm_from_date').val());	
	$('#print_to_date').val($('#dailyWm_to_date').val());	
	$('#print_nama_pegawai').val($('#dailyWm_nama_pegawai').val());	

    $('#wmreport-grid').yiiGridView('update', {
		data: $(this).serialize()
	});
	return false;
});
    ...
    $this->widget('bootstrap.widgets.TbGridView', array(
       ...
       'columns'=>array(
           ...
           array (
                ...
                'value'=>'$data->status == 1? '
                         . 'CHtml::ajaxLink("ACC", '
                                        . 'Yii::app()->createUrl("/daily/accWm", array("id" =>$data[\'daily_wm_id\'])),'
                                        . 'array ('
                                        .   '"type" => "GET",'
                                        .   '"dataType" => "json",'
                                        .   '"complete" => "'.'function(data){if(data.responseText==\'sukses update\') $.fn.yiiGridView.update(\'wmreport-grid\');}'.'",'
                                            . ')'
                                            . ')'
                . ': "-"',
            ), 
            array (
                ...
                'template' => '{delete}',
                'deleteButtonUrl'=>'Yii::app()->createUrl("/daily/deleteWm", array("id" =>$data[\'daily_wm_id\']))',
            )
           )	
       ));        
 } else { 
...
<?php $form=$this->beginWidget('bootstrap.widgets.TbActiveForm', array(
    'id'=>'csreport-print',
	'action'=>Yii::app()->createUrl('daily/printwm'),
	'method'=>'post',        
        'type'=>'horizontal',
        'htmlOptions'=> array(
            'target'=>"_blank",
...
<input name="dailyWm[kriteria_nasabah]" id="print_kriteria_nasabah" type="hidden" />
<input name="dailyWm[from_date]" id="print_from_date" type="hidden" />
<input name="dailyWm[to_date]" id="print_to_date" type="hidden" />
<input name="dailyWm[nama_pegawai]" id="print_nama_pegawai" type="hidden" />
<div class="form-actions">        	
        <?php $this->widget('bootstrap.widgets.TbButton', array(
                'buttonType'=>'submit',                
                'type'=>'danger',
